...
public class EditorScriptBlockBuilder : BlockBuilder
{
	...
	public override string Build(int3 startPos, IArgs? iArgs)
	{
	...
	private string BuildNormal(Block[] blocks)
	{
		...
		if (blocks.Length > 0 && blocks[0].Pos != int3.Zero)
		{
	...
	private string BuildBase64(Block[] blocks)
	{
		...
		{
			bool insertBlockAtZero = blocks.Length > 0 && blocks[0].Pos != int3.Zero;
