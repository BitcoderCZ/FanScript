// <copyright file="BoundConstructorExpression.cs" company="BitcoderCZ">
...
internal sealed class BoundConstructorExpression : BoundExpression
{
		...
		: base(syntax)
	{
		...
		{
			float3 val = new float3((float)ExpressionX.ConstantValue.GetValueOrDefault(TypeSymbol.Float), (float)ExpressionY.ConstantValue.GetValueOrDefault(TypeSymbol.Float), (float)ExpressionZ.ConstantValue.GetValueOrDefault(TypeSymbol.Float));
